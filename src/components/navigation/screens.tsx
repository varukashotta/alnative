...
const bottomMenuLinks = ({bottomLinks}: bottomLinksIP): any[] => {
    ...
    if(bottomLinks) {
        for (let i = 0; i < bottomLinks.length; i++) {
            ...
            navs.push(<Tab.Screen key={i} name={screenName} component={screen} options={{
                tabBarLabel: label,
                tabBarIcon: ({focused}) => (
...
const bottomScreen = ({bottomLinks, defaultScreen}: bottomLinksIP): ReactNode => {
    ...
    return <Stack.Screen name='Home' component={container} options={{headerShown: false}}/>
}
...
const stackScreens = ({screens}: StackScreensIP): any[] => {
    ...
    for (let i = 0; i < screens.length; i++) {
       ...
       visibilityState && renderedScreens.push(<Stack.Screen key={i} name={name} component={component} options={options} />)
    }
